<div id="page-credits" class="layout-col layout-col-full-width">

    <div class="layout-col-title">
                <span class="pull-left w60">
                    <i class="fa fa-question-circle"></i> Comunidade Minecraft Portugal
                </span>
                <span class="pull-right">
                    MinePanel 3.0 <i class="fa fa-question-circle"></i>
                </span>

        <div class="clearer"></div>
    </div>

    <div class="padded">
        <span class="w40pct pull-left center">
            <img src="/images/logo_xs.png"/>
        </span>
        <span class="pull-left w30pct">
            <dl class="credits">
                <dt><h1>web panel</h1></dt>

                <dt>design e programação</dt>
                <dd>Mário C. <span>neofreak</span></dd>

                <dt>artwork</dt>
                <dd>Joaquim B. <span>kyn</span></dd>
                <dd>T. Simões <span>_simoes_</span></dd>

                <dt>contribuições</dt>
                <dd>Hugo P. <span>Theorem</span></dd>
                <dd>Rui L. <span>tulicreme</span></dd>
                <dd>Vasco F.<span>dubaiss</span></dd>
            </dl>
        </span>
        <span class="pull-left w30pct">
            <dl class="credits">
                <dt><h1>minecraft server</h1></dt>
                <dt>direcção executiva</dt>
                <dd>Mário C. <span>neofreak</span></dd>
                <dd>João A. <span>Deevian</span></dd>

                <dt>programação</dt>
                <dd>João A. <span>Deevian</span></dd>
                <dd>João H. <span>disco</span></dd>
                <dd>Vasco F.<span>dubaiss</span></dd>
                <dd>Mário C. <span>neofreak</span></dd>

                <dt>administracao do servidor</dt>
                <dd>Mário C. <span>neofreak</span></dd>
                <dd>João A. <span>Deevian</span></dd>
                <dd>Joana F. <span>IronJoo</span></dd>
            </dl>
        </span>
    </div>

    <div class="clearer"></div>


    </div>

</div>